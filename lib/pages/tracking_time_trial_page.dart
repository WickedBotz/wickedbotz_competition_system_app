...
class TrackingTimeTrialPage extends StatefulWidget {
  ...
  const TrackingTimeTrialPage({super.key, required this.timeTrial, required this.Competiotion, required this.Category});
...
class _TrackingTimeTrialPage extends State<TrackingTimeTrialPage> {
  ...
  final pointsTimeTrialControllers = [
    TextEditingController(),
    TextEditingController(),
    TextEditingController(),
  ];
      ...
      children: List.generate(3, (index) {
        return Column(
          children: [
            ...
            TextFormField(
              controller: pointsTimeTrialControllers[index],
              keyboardType: TextInputType.numberWithOptions(decimal: true),
              inputFormatters: [
                FilteringTextInputFormatter.allow(RegExp(r'^[0-9]*$')),
              ],
                  ...
                  Column(
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Text('Pontos ${i + 1}: '),
                          Text((int.tryParse(pointsTimeTrialControllers[i].text) ?? 0).toString()),
                        ],),
  ...
  Future<bool> sendFormValues() async {
    ...
    final body = jsonEncode({
      ...
      'points1': int.tryParse(pointsTimeTrialControllers[0].text) ?? 0,
      'points2': int.tryParse(pointsTimeTrialControllers[1].text) ?? 0,
      'points3': int.tryParse(pointsTimeTrialControllers[2].text) ?? 0,
